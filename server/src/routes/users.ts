...
export const userRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/me', { preHandler: app.authenticate }, async (request) => {
    const user = await query('SELECT id,nickname,avatar_url,bio,role,profile_completed,created_at FROM users WHERE id=$1', [request.user.sub]);
    if (!user.rowCount) throw app.httpErrors.unauthorized('登录已失效，请重新登录');
  ...
  app.patch('/me', { preHandler: app.authenticate }, async (request) => {
    ...
    const result = await query(
      `UPDATE users SET nickname=$2,avatar_url=$3,bio=$4,profile_completed=true,updated_at=now() WHERE id=$1
       RETURNING id,nickname,avatar_url,bio,role,profile_completed`, [request.user.sub, body.nickname, body.avatarUrl ?? null, body.bio ?? null]
    );
    if (!result.rowCount) throw app.httpErrors.unauthorized('登录已失效，请重新登录');
  ...
  app.get('/me/growth-summary', { preHandler: app.authenticate }, async (request) => {
    ...
    const byGrade = await query(
      `SELECT r.grade,count(DISTINCT s.route_id)::int sends
       FROM sends s JOIN routes r ON r.id=s.route_id
       WHERE s.user_id=$1 AND s.moderation_status='approved'
       AND ($2::uuid IS NULL OR r.gym_id=$2) AND ($3::uuid IS NULL OR r.route_set_id=$3)
    ...
    const byGym = await query(
      `SELECT g.id gym_id,g.name gym_name,count(DISTINCT s.route_id)::int sends,
       ...
       FROM sends s JOIN routes r ON r.id=s.route_id JOIN gyms g ON g.id=r.gym_id
       WHERE s.user_id=$1 AND s.moderation_status='approved'
       GROUP BY g.id ORDER BY sends DESC`, [request.user.sub]
    );
  ...
  app.get('/me/cycle-summary', { preHandler: app.authenticate }, async (request) => {
    const result = await query(
      `SELECT g.id gym_id,g.name gym_name,rs.id route_set_id,rs.name route_set_name,
       ...
       WHERE s.user_id=$1 AND s.moderation_status='approved' AND rs.active=true
  ...
  app.post('/:id/friend-request', { preHandler: app.authenticate }, async (request) => {
    ...
    return transaction(async (client) => {
      ...
      const created = await client.query<{ requester_id: string; addressee_id: string; status: string }>(
      ...
      if (created.rowCount) {
        await client.query(
          `INSERT INTO notifications(user_id,type,title,content,target_path)
           VALUES($1,'friend_request','新的岩友申请','有人想添加你为岩友','/pages/friends/index')`,
          [id]
        );
        return { status: 'pending' };
      }

      const existing = await client.query<{ requester_id: string; addressee_id: string; status: string }>(
        `SELECT requester_id,addressee_id,status FROM friendships
         WHERE (requester_id=$1 AND addressee_id=$2) OR (requester_id=$2 AND addressee_id=$1)`,
        [request.user.sub, id]
      );
      const friendship = existing.rows[0];
      if (!friendship) throw app.httpErrors.conflict('好友关系状态已变化，请重试');
      if (friendship.status === 'accepted') return { status: 'accepted' };
      if (friendship.status === 'blocked') return { status: 'blocked' };
      return { status: friendship.requester_id === request.user.sub ? 'pending' : 'received' };
    });
  });
  app.post('/:id/friend-accept', { preHandler: app.authenticate }, async (request) => {
    ...
    return transaction(async (client) => {
      ...
      const friendship = existing.rows[0];
      if (!friendship || friendship.status === 'blocked') throw app.httpErrors.notFound('好友申请不存在');
      if (friendship.status === 'accepted') return { status: 'accepted' };
      ...
      await client.query(
        `INSERT INTO notifications(user_id,type,title,content,target_path)
         VALUES($1,'friend_accepted','岩友申请已通过','你们现在可以一起约爬了','/pages/friends/index')`,
        [id]
      );
